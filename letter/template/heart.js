var heart = function () {
    this.canvas = document.getElementById("heart_c");
    ...
    this.style_color = find_random(0, 1);
    var all_element = [];

    window_resize();

    this.start_heart = function () {
        window.requestAnimationFrame(this.start_heart);
        // style_color += 0.1;
        //ctx.fillStyle = "#ffffff00";//'hsl(' + style_color + ',100%,97%)';
    ...
    ready_run.prototype = {
        to_reset: function () {
            ...
            t.size_change = all_attribute.size_add; //find_random(all_attribute.size_add_min, all_attribute.size_add_max);
            t.opacity = find_random(all_attribute.opacity_min, all_attribute.opacity_max);
            t.opacity_change = all_attribute.opacity_prev; //find_random(all_attribute.opacity_prev_min, all_attribute.opacity_prev_max);
            t.light = find_random(all_attribute.light_min, all_attribute.light_max);
            t.color = '#ff0000'; //'hsl(' + style_color + ',100%,' + t.light + '%)';
        },
        to_step: function () {
            var t = this;
